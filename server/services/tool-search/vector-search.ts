...
function buildSearchableText(tool: ToolMetadata): string {
	return [tool.name.replace(/_/g, " "), tool.phrases.join(" ")].join(" ");
}
...
export async function initToolVectorIndex(
	...
	const toolList = tools || Object.values(TOOL_REGISTRY);
...
export async function vectorSearch(
	...
	return scores.slice(0, limit).map(({ name, score }) => {
		...
		return {
			...
			score,
			relatedTools:
				metadata.relatedTools.length > 0 ? metadata.relatedTools : undefined,
		};
	});
}
