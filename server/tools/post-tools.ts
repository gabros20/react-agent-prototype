...
export const cmsUpdatePost = tool({
		...
		updates: z
			.object({
				...
				content: z
					...
					.optional()
					.describe("New content (if updating body/cover/tags)"),
			})
	...
	execute: async (input, { experimental_context }) => {
		...
		if (
			input.updates.title ||
			input.updates.author ||
			input.updates.excerpt ||
...
export const cmsListPosts = tool({
	description:
		"List posts with title, image paths, excerpt, slug, category, published date. Default: published only. Image paths are pre-resolved /uploads/ URLs ready for markdown rendering.",
	inputSchema: z.object({
	...
	execute: async (input, { experimental_context }) => {
		...
		if (input.category) {
			...
			entries = await ctx.services.entryService.getEntriesByCategory(collection.id, input.category, input.localeCode || "en");
		} else if (input.status === "published" || input.status === undefined) {
			// Published posts only (default)
			entries = await ctx.services.entryService.listPublishedEntries(collection.id, input.localeCode || "en");
		} else if (input.status === "all") {
			// All posts (draft, published, archived)
			entries = await ctx.services.entryService.getCollectionEntries(collection.id, true, input.localeCode || "en");
		} else {
			// Specific status filter
			const allEntries = await ctx.services.entryService.getCollectionEntries(collection.id, true, input.localeCode || "en");
			entries = allEntries.filter((e: any) => e.status === input.status);
		}

		// Image paths are pre-resolved at write time - just return stored values
		const posts = entries.map((entry: any) => {
			...
			return {
				...
				featuredImage: entry.featuredImage, // Pre-resolved /uploads/ path
				coverImage: cover || null, // Pre-resolved url in cover.url
