			...
			<div className='flex-none p-3 sm:p-4 border-b bg-muted/30'>
				<div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
					<div className='flex items-center gap-2 flex-1 min-w-0'>
						<Terminal className='h-5 w-5 text-primary flex-none' />
						<div className='flex-1 min-w-0'>
							<h2 className='text-base sm:text-lg font-semibold'>Execution Log</h2>
				...
				<div className={`flex items-start gap-2 p-2 sm:p-3 ${log.input || log.output ? "cursor-pointer" : ""}`}>
					...
					<div className='flex-1 min-w-0'>
						<div className='flex flex-wrap items-center gap-1 sm:gap-2'>
							{log.toolName && <span className='font-mono text-xs sm:text-sm truncate'>{log.toolName}</span>}
							<span className='text-[10px] sm:text-xs text-muted-foreground flex-shrink-0'>
								{new Date(log.timestamp).toLocaleTimeString()}
							</span>
						</div>
						{log.message && <p className='text-xs sm:text-sm text-muted-foreground mt-1 break-words'>{log.message as string}</p>}
					</div>
					{(log.input !== undefined || log.output !== undefined) && (
				...
				{log.input ? (
					<div className='mt-2'>
						...
						<pre className='text-[10px] sm:text-xs bg-muted p-2 rounded overflow-x-auto max-h-48 overflow-y-auto'>
							{formatJson(log.input) as string}
						</pre>
					</div>
				) : null}
				{log.output ? (
					<div className='mt-2'>
						...
						<pre className='text-[10px] sm:text-xs bg-muted p-2 rounded overflow-x-auto max-h-48 overflow-y-auto'>
							{formatJson(log.output) as string}
						</pre>
					</div>
				) : null}
			</CollapsibleContent>
