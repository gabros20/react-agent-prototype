...
async function seed() {
	...
	try {
		...
		// Resolve image UUIDs to actual file paths from the images table
		// seed-images.ts runs before seed.ts, so images should already exist
		const images = await db.query.images.findMany({
			where: inArray(schema.images.id, seedImageIds),
			columns: { id: true, filePath: true },
		});

		// Build a map of UUID -> /uploads/ path
		const imagePathMap = new Map<string, string>();
		for (const img of images) {
			if (img.filePath) {
				imagePathMap.set(img.id, `/uploads/${img.filePath}`);
			}
		}

		// Helper to get resolved image path (falls back to placeholder if not found)
		const getImagePath = (uuid: string, fallback: string): string => {
			return imagePathMap.get(uuid) || fallback;
		};

		// Resolved image paths for blog posts
		const seedImagePaths = [
			getImagePath(seedImageIds[0], "/assets/images/placeholders/mountain-placeholder.jpg"),
			getImagePath(seedImageIds[1], "/assets/images/placeholders/puppy-placeholder.jpg"),
			getImagePath(seedImageIds[2], "/assets/images/placeholders/workspace-placeholder.jpg"),
		];

		if (images.length > 0) {
			console.log(`✓ Resolved ${images.length} image paths from database`);
		} else {
			console.log(`⚠️  No images found in database, using placeholder paths`);
		}
		...
		await db.insert(schema.pageSectionContents).values({
			...
			content: JSON.stringify({
				heading: "Our Mission",
				content:
					"We believe content creation should be intuitive, powerful, and accessible to everyone. That's why we built a CMS that combines the best of AI technology with human creativity.\n\nOur platform empowers teams to focus on what matters most: creating exceptional content that resonates with their audience.",
				image: {
					url: "/uploads/images/2025/11/23/original/7f27cf0e-0b38-4c24-b6c5-d15528c80ee3.jpg",
					alt: "Our mission",
				},
		...
		await db.insert(schema.collectionEntries).values({
			...
			featuredImage: seedImagePaths[0], // Mountain landscape
			category: "Tutorials",
...
Happy building! 🚀`,
				cover: {
					url: seedImagePaths[0], // Mountain landscape
		...
		await db.insert(schema.collectionEntries).values({
			...
			featuredImage: seedImagePaths[1], // Golden puppy
			category: "Advanced",
...
These techniques will help you build production-ready, high-performance sites. Experiment and find what works best for your use case!`,
				cover: {
					url: seedImagePaths[1], // Golden puppy
		...
		await db.insert(schema.collectionEntries).values({
			...
			featuredImage: seedImagePaths[2], // Desk workspace
			category: "News",
...
Stay tuned for more updates! 🎉`,
				cover: {
					url: seedImagePaths[2], // Desk workspace
