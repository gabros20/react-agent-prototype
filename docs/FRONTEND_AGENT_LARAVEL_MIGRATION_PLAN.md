...
16. `search_vector` - Semantic vector search (LanceDB)
17. `cms_findResource` - Fuzzy resource search (typo-tolerant)

**HTTP Tools (2 tools)**:
18. `http_get` - External HTTP GET
19. `http_post` - External HTTP POST (requires approval)

**Planning Tools (1 tool)**:
20. `plan_analyzeTask` - Task planning and analysis
...
export const semanticSearchTool = tool({
  ...
  execute: async ({ query, limit }) => {
    const results = await client.collections('pages')
      .documents()
      .search({
        q: query,
...
const handleApprove = async () => {
  ...
  laravelApi.post('/api/audit/approvals', {
    tool: pendingApproval.toolName,
  ...
  execute: async (input, { logger }) => {
    ...
    try {
      ...
      if (input.id) {
        ...
        const pages = await laravelApi.get(
        ...
        if (input.includeContent) {
          page = await laravelApi.get(
            cmsContext.cmsPath(`pages/${page.id}/contents`),
...
describe('cmsGetPageTool', () => {
  it('fetches page by slug', async () => {
    ...
    expect(laravelApi.get).toHaveBeenCalledWith(
