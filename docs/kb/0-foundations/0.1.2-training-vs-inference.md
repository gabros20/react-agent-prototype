...
**Versions**: OpenAI GPT-4o, Anthropic Claude 3.5, Google Gemini 2.5, Meta Llama 3
...
```mermaid
flowchart TD
    A[Training Phase] -->|One-time: $1M-$100M| B[Pre-trained Model]
    B -->|Deploy| C[Production API]
    C -->|Millions of calls| D[Inference Phase]

    A -->|Weeks to months| A1[10,000+ GPUs]
    A -->|Trillions of tokens| A2[Massive Datasets]

    D -->|Milliseconds to seconds| D1[Single GPU per request]
    D -->|User input only| D2[$0.0001-$0.10 per request]
...
| Model          | Parameters | Training Cost | Hardware         | Training Time | GPU Hours   |
| -------------- | ---------- | ------------- | ---------------- | ------------- | ----------- |
| GPT-3          | 175B       | $4.6M         | 10,000 V100 GPUs | 1 month       | ~355 years  |
| GPT-4          | ~1.8T      | $100M         | Classified       | 3-6 months    | Undisclosed |
| Llama 3.1 405B | 405B       | $5-10M        | 16,000 H100 GPUs | 21 days       | 30.8M hours |
| Gemini Ultra   | ~200B      | $191M         | Google TPUv5     | 2-4 months    | Undisclosed |
| DeepSeek v3    | 671B       | $5.6M         | 2,048 H800 GPUs  | ~2 months     | 2.79M hours |
...
sequenceDiagram
    ...
    Model->>Model: Generate multiple responses to prompt
    Model->>Human: Show responses for ranking
    Human->>Human: Rank responses (best to worst)
    Human->>RewardModel: Provide rankings
    RewardModel->>RewardModel: Learn to predict human preferences
    RewardModel->>Model: Train model via reinforcement learning
    Model->>Model: Update weights to maximize reward
```
...
flowchart LR
    ...
    B --> C[Forward Pass<br/>through all layers]
...
| -------------------------- | -------------------------- | --------------------------- | ----------------------- |
| GPT-4o                     | $2.50                      | $10.00                      | High quality, reasoning |
| GPT-4o-mini                | $0.15                      | $0.60                       | Fast, cost-effective    |
| Claude 3.5 Sonnet          | $3.00                      | $15.00                      | Long context, coding    |
| Gemini 2.5 Flash           | $0.10                      | $0.30                       | Ultra-fast, cheapest    |
| Llama 3.1 70B (OpenRouter) | $0.50                      | $0.50                       | Open weights, balanced  |
...
**Impact**: This codebase reduced context size by 80-90% through hybrid content fetching.

**Source**: [PROGRESS.md Sprint 15][^12]
...
**Source**: [AGENTIC_PATTERNS_LIBRARY.md Pattern #1][^13]
...
This codebase uses API-based inference via OpenRouter, enabling flexible model selection:

```typescript
// server/agent/orchestrator.ts
import { openrouter } from "@openrouter/ai-sdk-provider";

const model = openrouter.languageModel(process.env.OPENROUTER_MODEL || "google/gemini-2.5-flash");
```

**Model Configuration**: [.env.example](../../.env.example)
...
**File**: [server/tools/all-tools.ts](../../server/tools/all-tools.ts)

Implements hybrid content fetching to reduce token usage:

```typescript
// Lazy-load content only when needed
export const cms_getPage = defineTool({
	name: "cms_getPage",
	parameters: z.object({
		slug: z.string(),
	...
	execute: async ({ slug, includeContent }) => {
		...
		if (includeContent) {
			// Load full content (expensive)
			return { ...page, content: await fetchContent(page.id) };
		}
...
// Savings: 87% token reduction (2,000 → 100 tokens) ✅
```
...
1. **Implement Prompt Caching**: Enable OpenAI/Anthropic prompt caching for 44% cost savings
    ...
    - **Current**: System prompt sent fresh every request (2,300 tokens)
    - **Recommended**: Cache system prompt + tool definitions (90% discount after first request)
    - **Benefit**: $10/month → $6/month (40% savings)

2. **Add Context Compression**: Summarize old conversation history when >80% context capacity

    - **Current**: Full conversation history sent every time (grows linearly)
    - **Recommended**: Compress first 70% of messages into summary (10:1 ratio)
...
3. **Model Tier Routing**: Route simple tasks to cheaper models (Gemini Flash), complex to GPT-4o
    - **Current**: All tasks use same model (Gemini 2.5 Flash)
    - **Recommended**: Classify task complexity, route to appropriate tier
...
12. **Project Documentation**. "PROGRESS.md Sprint 15: Hybrid Content Fetching & Token Optimization". [../../PROGRESS.md](../../PROGRESS.md#sprint-15-hybrid-content-fetching-token-optimization)
13. **Project Documentation**. "AGENTIC_PATTERNS_LIBRARY.md Pattern #1: Hierarchical Memory". [../../AGENTIC_PATTERNS_LIBRARY.md](../../AGENTIC_PATTERNS_LIBRARY.md#1-hierarchical-memory-subgoal-based)
14. **OpenAI** (2024). "Prompt Caching Documentation". https://platform.openai.com/docs/guides/prompt-caching
...
[^12]: ../../PROGRESS.md#sprint-15-hybrid-content-fetching-token-optimization
[^13]: ../../AGENTIC_PATTERNS_LIBRARY.md#1-hierarchical-memory-subgoal-based
