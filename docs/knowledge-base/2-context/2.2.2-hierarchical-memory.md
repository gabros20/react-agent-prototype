...
Hierarchical memory organizes agent context into subgoal-based chunks with automatic compression—achieving 2× success rates and 85-95% context reduction by summarizing completed subgoals instead of retaining full action-observation histories.

- **Status**: ✅ Complete
- **Last Updated**: 2025-12-03
- **Prerequisites**: [2.2.1 Sliding Window](./2.2.1-sliding-window.md)
- **Grounded In**: HiAgent (ACL 2025) - 2× success rate, 35% context reduction
...
- **2× success rate**: HiAgent shows 100% improvement in long-horizon tasks (ACL 2025)
...
- Simple task (10 steps): 10 × 200 tokens = 2,000 tokens
- Complex task (50 steps): 50 × 200 tokens = 10,000 tokens
- Long-running task (200 steps): 200 × 200 tokens = 40,000 tokens
```

**Problems**:

- ❌ Context grows unbounded with task complexity
- ❌ Most historical details become irrelevant once subgoal completes
- ❌ "Lost in the middle" effect—models ignore mid-context information
- ❌ Longer prompts = slower inference and higher costs
...
**Result**: 7.5× reduction while preserving essential information.
...
```
┌─────────────────────────────────────────┐
│  LEVEL 0: Task Objective                │
│  "Build comprehensive docs site"        │
│  (~100-200 tokens, always in context)   │
└─────────────────────────────────────────┘
              ↓
┌─────────────────────────────────────────┐
│  LEVEL 1: Completed Subgoals (Summary)  │
│  ✅ Foundation pages (3 pages)          │
│  ✅ Core docs (5 pages)                 │
│  ✅ Reference materials (2 pages)       │
│  (~100-200 tokens each)                 │
└─────────────────────────────────────────┘
              ↓
┌─────────────────────────────────────────┐
...
│  - cms_createPage("Advanced Topics")    │
│  - cms_updatePage(slug: "advanced")     │
│  (~200-500 tokens per action)           │
└─────────────────────────────────────────┘
              ↓
┌─────────────────────────────────────────┐
│  LEVEL 3: Trajectory Retrieval          │
│  (On-demand, temporary injection)       │
│  Retrieved for specific queries only    │
└─────────────────────────────────────────┘
```
...
│     Analyze task → Generate next subgoal │
│     Define success criteria              │
└──────────────────────────────────────────┘
              ↓
┌──────────────────────────────────────────┐
│  2. ACTION GENERATION                    │
│     Select tools → Execute actions       │
...
│     Trigger summarization when complete  │
└──────────────────────────────────────────┘
              ↓
┌──────────────────────────────────────────┐
│  4. SUMMARIZATION                        │
│     Compress action-observation pairs    │
│     Store summary, discard details       │
...
1. **Abstraction Hierarchy**: Memory organized by granularity (task → phase → subgoal → action)
...
- ✅ Immediate compression on subgoal completion
...
**Pros**:
- ✅ Preserves critical IDs/references (extractive)
- ✅ Natural language readability (abstractive)
- ✅ Best accuracy for production

**Cons**:
- ❌ Requires LLM call for narrative
...
**Pros**:
- ✅ Robust detection with hybrid approach
- ✅ No LLM call needed for most cases
- ✅ Handles both explicit and implicit completion

**Cons**:
- ❌ May miss subtle completion signals
- ❌ Requires tuning detection patterns

**When to Use**: Any system with hierarchical memory—detection accuracy directly impacts compression timing.
...
- **Results**: 2× success rate, 35% context reduction, 19.42% faster runtime
...
1. SUBGOAL COMPLETION (most common)
   └── Success criteria met → Compress immediately
   └── Frequency: Every 3-10 steps

2. CONTEXT CAPACITY (safety net)
   └── Context reaches 80% → Compress oldest subgoals
   └── Prevents overflow even if detection fails

3. PHASE TRANSITION (strategic)
   └── Task enters new phase → Compress all prior phase
...
- ❌ Too small: "Create one page" (trivial, adds overhead)
- ❌ Too large: "Build entire site" (that's the whole task)
- ✅ Just right: "Create foundation pages (intro, getting-started, overview)"
...
#### ❌ Pitfall 1: Losing Critical References

Summarization can lose IDs needed for future actions.

**Problem**: "Page created with ID 123" → "Page created successfully" (loses ID)
...
- Action-observation history: 50 × 400 = 20,000 tokens
...
- 5 subgoal summaries: 5 × 200 = 1,000 tokens
- Current subgoal (8 actions): 8 × 400 = 3,200 tokens
...
2. **2× success rate improvement**: Validated by HiAgent research
...
- [2.2.5 Prompt Caching](./2.2.5-prompt-caching.md) - Cache-aware summary injection patterns
