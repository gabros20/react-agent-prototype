...
#### 1. Exact Action Repetition Detector

```typescript
// server/agent/stuck-detection.ts
...
#### 2. Progress-Based Detection

```typescript
// server/agent/progress-detector.ts
...
#### 3. Composite Detector

```typescript
// server/agent/composite-stuck-detector.ts
  ...
  const result = await generateText({
    ...
    experimental_onStepFinish: async (step) => {
      ...
      if (status.isStuck) {
        ...
        // Option 1: Throw to stop immediately
        // throw new StuckLoopError(status.reason, status.details);

        // Option 2: Add guidance to next step
        // This could help the agent break out of the loop
...
```typescript
// server/agent/stuck-recovery.ts
...
export async function runProductionAgent(
  prompt: string,
  context: AgentContext,
  config: StuckDetectionConfig = DEFAULT_CONFIG
): Promise<AgentResult> {
  const detector = new CompositeStuckDetector(config);
  const recovery = config.enableHITL
    ? new HITLRecoveryStrategy()
    : new ImmediateTerminationStrategy();
  ...
  try {
    const result = await generateText({
      ...
      experimental_onStepFinish: async (step) => {
        ...
        if (status.isStuck) {
          context.logger?.warn('Stuck detected', status);

          if (config.enableHITL) {
            const decision = await recovery.handleStuck(status, context);

            if (decision === 'abort') {
              throw new StuckLoopError('User aborted', status.details);
            }
            // Continue if 'continue' or 'modify'
          } else {
            throw new StuckLoopError(status.reason, status.details);
          }
        }
    ...
    if (error instanceof StuckLoopError) {
      return {
        ...
        suggestion: 'Try rephrasing with more specific instructions',
      };
    }
    throw error;
  }
...
```typescript
// server/agent/__tests__/stuck-detection.test.ts
...
describe('ProgressBasedStuckDetector', () => {
  it('detects high error rate', () => {
    ...
    // 3 errors out of 3 steps = 100% error rate
...
// Detection Strategy
const strategy = {
  checkFrequency: 'every_step',
  recovery: 'hitl',               // Or 'abort' for immediate stop
  fallback: 'abort',
};
...
3. **ReflAct Paper** - "World-Grounded Decision Making via Goal-State Reflection" (May 2025)
...
5. **AI SDK v6** - "Multi-Step Calls and Step Callbacks" (2025)
