...
#### 1. Entropy-Based Exit (HALT-CoT Method)

```typescript
// server/agent/entropy-exit.ts
...
#### 2. Progress-Based Exit

```typescript
// server/agent/progress-exit.ts
...
#### 3. Stop-RAG (Value-Based Retrieval)

```typescript
// server/agent/stop-rag.ts
...
```typescript
// server/agent/early-exit-manager.ts
...
```typescript
// server/agent/__tests__/early-exit.test.ts
...
2. **HALT-CoT** - "Model-Agnostic Early Stopping via Answer Entropy" (Jul 2025)
...
4. **Stop-RAG** - "Value-Based Retrieval Control for Iterative RAG" (Oct 2025)
   https://www.arxiv.org/abs/2510.14337

5. **Learning When to Plan** - "Efficiently Allocating Test-Time Compute" (Sep 2025)
