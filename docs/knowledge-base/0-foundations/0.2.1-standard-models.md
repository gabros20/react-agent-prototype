...
-   **Grounded In**: OpenAI o1/o3/o4-mini research (2024-2025), DeepSeek-R1-0528 (2025), Qwen3 technical report (2025), ARC-AGI benchmarks
...
**As of November 2025**: Reasoning models have matured significantly, with multiple providers (OpenAI, DeepSeek, Alibaba Qwen, Moonshot AI) offering both commercial and open-source options. OpenAI released o4-mini in early 2025, providing cost-efficient reasoning capabilities at 21% ARC-AGI-1 accuracy for ~5 cents per task.

### Key Research Findings (2024-2025)

-   **Math Performance (AIME 2024)**: o1 achieves 83% vs GPT-4o's 13% (6.4× improvement)
-   **Math Performance (AIME 2025)**: DeepSeek-R1-0528 achieves 87.5%, Qwen3-235B achieves 81.4%
...
-   **AGI Reasoning (ARC-AGI-1)**: o3 achieves 87.5% (high-compute), o3-medium 53%, o4-mini-medium 41% vs 0% for GPT-3 in 2020
-   **ARC-AGI-2** (introduced March 2025): o3/o4-mini score <3% vs 60% for average humans (significant gap remains)
...
-   `sendReasoning: true`: Stream reasoning tokens to client for real-time display
...
export async function POST(req: Request) {
	...
	// Stream reasoning tokens to client
...
-   **Results**:
    ...
    -   ARC-AGI-1: 87.5% (o3 high-compute), 53% (o3-medium), 41% (o3-low) vs 5% (GPT-4o)
    -   ARC-AGI-2: <3% (o3) vs 60% (average human) - significant gap remains

**Source**: [OpenAI o1 Announcement](https://openai.com/index/introducing-openai-o1-preview/)

### OpenAI o3 and o4-mini (December 2024 - Early 2025)

**Paper**: "Analyzing o3 and o4-mini with ARC-AGI"
...
-   **o4-mini Performance**: 21% (o4-mini-low) and 41% (o4-mini-medium) on ARC-AGI-1
-   **Cost Efficiency**: o4-mini achieves 21% accuracy at ~5 cents per task vs $11+ for o1-pro
-   **ARC-AGI-2 Limitation**: Only 3% on ARC-AGI-2 (more difficult benchmark), compared to 60% for average humans
-   **Cost (o3)**: $17-20 and 33 million tokens per puzzle (low-compute configuration)
...
-   **Key Innovation**: Open-source reasoning model with major performance improvements in R1-0528 update
-   **Results**:
    -   AIME 2024: 91.4% (R1-0528) vs 79.8% (original R1)
    -   AIME 2025: 87.5% (R1-0528) vs 70.0% (original R1)
    -   MATH-500: 97.3%
    -   MMLU: 90.8%
    -   GPQA Diamond: 71.5%
    -   Codeforces: 96.3% percentile (expert-level)
-   **Advantage**: Open-source, customizable, significant improvement trajectory
...
-   **Results**:
    -   AIME 2024: 85.7% (competitive with top models)
    -   AIME 2025: 81.4% vs 81.5% baseline
    -   Outperforms DeepSeek-R1, Grok-3, and o3-mini on AIME'24
...
| Benchmark                    | GPT-4o          | o1              | o3 (high) | o4-mini (medium) | DeepSeek-R1-0528  | Qwen3-235B  |
| ---------------------------- | --------------- | --------------- | --------- | ---------------- | ----------------- | ----------- |
| **AIME 2024 (Math)**         | 13.4%           | 83%             | 96.7%     | N/A              | 91.4%             | 85.7%       |
| **AIME 2025 (Math)**         | N/A             | N/A             | N/A       | N/A              | 87.5%             | 81.4%       |
| **GPQA (Graduate Science)**  | 56%             | 86%             | N/A       | N/A              | 71.5%             | N/A         |
| **Codeforces (Programming)** | 11th percentile | 93rd percentile | N/A       | N/A              | 96.3rd percentile | Leading Elo |
| **MMLU (General Knowledge)** | 88%             | 92%             | N/A       | N/A              | 90.8%             | Competitive |
| **ARC-AGI-1 (Reasoning)**    | 5%              | N/A             | 87.5%     | 41%              | N/A               | N/A         |
| **ARC-AGI-2 (Hard)**         | <3%             | N/A             | <3%       | <3%              | N/A               | N/A         |
| **Cost (per 1M tokens)**     | $2.50-$10       | $150-$600       | N/A       | ~$50 (est)       | $550-$2,190       | Open-source |

**Key Insight**: Reasoning models excel at **complex reasoning tasks** (math, science, code) but are only marginally better at **factual knowledge**. ARC-AGI-2 (March 2025) reveals significant gaps between AI (3%) and human reasoning (60%).
...
| ------------------------------------ | ------------------------- |
| Math proofs, scientific calculations | o3 or DeepSeek-R1-0528    |
...
5. **Cost-Efficient Options**: o4-mini provides 21% ARC-AGI-1 accuracy at ~5 cents per task
...
5. **ARC-AGI-2 Gap**: Even best models (o3) score <3% vs 60% for humans on hardest reasoning benchmarks
...
5. **Open-source alternatives exist** - DeepSeek-R1-0528 and Qwen3 match commercial performance at lower cost
6. **o4-mini provides cost-efficient reasoning** - 21% ARC-AGI-1 accuracy at ~5 cents per task
...
-   [ ] Consider o4-mini for cost-sensitive reasoning workloads
...
3. **ARC Prize Team** (2025). "Analyzing o3 and o4-mini with ARC-AGI", ARC Prize. [https://arcprize.org/blog/analyzing-o3-with-arc-agi](https://arcprize.org/blog/analyzing-o3-with-arc-agi)
...
9. **Qwen Team** (2025). "Qwen3: Think Deeper, Act Faster", Qwen Blog. [https://qwenlm.github.io/blog/qwen3/](https://qwenlm.github.io/blog/qwen3/)

### Benchmarks & Analysis

10. **DataCamp** (2025). "OpenAI's O3: Features, O1 Comparison, Benchmarks & More". [https://www.datacamp.com/blog/o3-openai](https://www.datacamp.com/blog/o3-openai)

11. **DataCamp** (2025). "DeepSeek-R1: Features, o1 Comparison, Distilled Models & More". [https://www.datacamp.com/blog/deepseek-r1](https://www.datacamp.com/blog/deepseek-r1)

12. **DataCamp** (2025). "Qwen3: Features, DeepSeek-R1 Comparison, Access, and More". [https://www.datacamp.com/blog/qwen3](https://www.datacamp.com/blog/qwen3)

13. **VentureBeat** (2024). "OpenAI's o3 shows remarkable progress on ARC-AGI, sparking debate on AI reasoning". [https://venturebeat.com/ai/openais-o3-shows-remarkable-progress-on-arc-agi-sparking-debate-on-ai-reasoning](https://venturebeat.com/ai/openais-o3-shows-remarkable-progress-on-arc-agi-sparking-debate-on-ai-reasoning)

14. **Helicone** (2025). "OpenAI o3 Released: Benchmarks and Comparison to o1". [https://www.helicone.ai/blog/openai-o3](https://www.helicone.ai/blog/openai-o3)

### Community Resources

15. **Medium** (2025). "DeepSeek's New R1-0528: Performance Analysis and Benchmark Comparisons". [https://medium.com/@leucopsis/deepseeks-new-r1-0528-performance-analysis-and-benchmark-comparisons-6440eac858d6](https://medium.com/@leucopsis/deepseeks-new-r1-0528-performance-analysis-and-benchmark-comparisons-6440eac858d6)

16. **PromptLayer** (2025). "OpenAI o3 vs DeepSeek r1: Which Reasoning Model is Best?". [https://blog.promptlayer.com/openai-o3-vs-deepseek-r1-an-analysis-of-reasoning-models/](https://blog.promptlayer.com/openai-o3-vs-deepseek-r1-an-analysis-of-reasoning-models/)

17. **FlowHunt** (2024). "RAG with Reasoning LLMs: OpenAI o1 vs OpenAI GPT-4o". [https://flowhunt.io/blog/rag-with-reasoning-llms-openai-o1-vs-openai-gpt4o](https://flowhunt.io/blog/rag-with-reasoning-llms-openai-o1-vs-openai-gpt4o)

18. **LessWrong** (2024). "o1: A Technical Primer". [https://www.lesswrong.com/posts/byNYzsfFmb2TpYFPW/o1-a-technical-primer](https://www.lesswrong.com/posts/byNYzsfFmb2TpYFPW/o1-a-technical-primer)
