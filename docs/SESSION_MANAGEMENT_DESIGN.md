...
router.delete("/sessions/:sessionId", async (req, res) => {
	...
	const messagesDeleted = await db.delete(schema.messages).where(eq(schema.messages.sessionId, sessionId));

	// 2. Delete session (also clears checkpoint JSON)
	const sessionDeleted = await db.delete(schema.sessions).where(eq(schema.sessions.id, sessionId));
